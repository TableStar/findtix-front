...
const EventPage = () => {
  ...
  const [openChk, setOpenChk] = React.useState(false);
  const index = 0;
  ...
  useEffect(() => {
    if (databaseEvent.length) {
      setEventTime({
        date: new Date(databaseEvent[0].startDate).toLocaleDateString("en-us", {
          weekday: "long",
          month: "long",
          year: "numeric",
          day: "numeric",
        }),
        startTime: databaseEvent[0].startDate
          .split("T")[1]
          .split(".")[0]
          .split(":"),
        endTime: databaseEvent[0].endDate
          .split("T")[1]
          .split(".")[0]
          .split(":"),
        eventDuration:
          (new Date(databaseEvent[0].endDate) -
            new Date(databaseEvent[0].startDate)) /
          1000 /
          60,
      });
    }
  ...
  if (databaseEvent.length && eventTime.date) {
    const printTags = () => {
      return databaseEvent[0].tags.map((value, index) => (
        <span
          key={index}
          className="bg-[#f4f2f8] text-gray-500 py-2 px-3 rounded-full"
        >
          #{value}
        </span>
      ));
        ...
        <div className="event-details-page">
          <div className="relative bg-gray-100 md:w-full md:h-[500px]">
            <div className="event-background-image-container w-[90%] bg-white rounded-[25px] absolute left-0 right-0 m-auto top-10">
              <div
                className={`event-image-container md:w-[100%] md:h-[500px] md:m-auto`}
                style={{ backgroundImage: `url(${databaseEvent[index].img})` }}
              ></div>
            </div>
            <img
              src={databaseEvent[index].img}
              className="md:h-[500px] md:m-auto md:absolute md:left-0 md:right-0 md:top-10 object-cover"
            />
          </div>
          ...
          <div className="event-contents flex justify-between md:mt-[65px]">
            <div className=" md:w-[60%] flex flex-col gap-6 p-6 lg:px-12">
              <div className="event-headline relative flex flex-col gap-3">
                ...
                <div className="bg-gray-100 w-full md:w-[75%] lg:w-[50%] flex flex-col gap-2 rounded-lg py-3 px-6 mt-3">
                  <p>
                    By{" "}
                    <span className="font-medium cursor-pointer">Creator</span>
                  </p>
              ...
              <div className="event-details flex flex-col gap-8">
                ...
                <div className="event-about">
                  ...
                  <div className="flex flex-col gap-6 mt-1">
                    ...
                    <p className="text-justify text-gray-500 lg:text-lg">
                      {databaseEvent[index].description}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="event-tags">
                ...
                <div className="flex flex-wrap gap-4 mt-4 text-xs lg:text-sm">
                  <span className="bg-[#f4f2f8] text-gray-500 py-2 px-3 rounded-full">
                    {databaseEvent[index].category.name} Events
                  </span>
            ...
            <div className="checkout-button flex flex-col justify-center w-full md:w-[400px] md:h-fit fixed md:sticky md:top-[80px] md:rounded-lg md:mt-[24px] md:mr-12 bottom-0 bg-white p-6 gap-4">
              {databaseEvent[index].ticketTypes.length > 1 ? (
                <div className="border-[2px] flex flex-col p-4 border-blue-700 rounded-md gap-6">
                  ...
                  <div className="flex gap-4">
                    <span className="font-medium">
                      {databaseEvent[index].ticketTypes[0].name}
                    </span>
